                     ...
                     int       nCmdShow)
{
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_TESTHOOK));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_TESTHOOK);
...
void OnAdd(HWND hwnd)
{
	...
	inshook insthook;
	hinst = LoadLibrary(_T("hackLync.dll"));
	if (hinst == NULL)
	{
		MessageBox(hwnd, _T("no Hook.dll!"), _T("no hook"), MB_OK);
		return;
	}
	insthook = ::GetProcAddress(hinst, "InstallHook");
	if(insthook == NULL)
	{
		MessageBox(hwnd, _T("func not found!"), _T("func not found"), MB_OK);
		return;
	}
	insthook();
}
